...
with st.sidebar:
    ...
    st.button("저장", width='stretch')

    st.markdown("#### 전체 설비 교체일 관리")
    st.dataframe(EQUIP_LIST, hide_index=True, width='stretch', height=210)
...
with h3:
    st.button("기간 선택", type="primary", width='stretch')
with h4:
    st.markdown(
...
# ────────────────────────────────────────────────────────────────────────────
# 2. Swap Delay Time Trend (전체 폭 · 우측에 설비 선택 버튼)
# ────────────────────────────────────────────────────────────────────────────
chart_col, side_col = st.columns([5, 1])

with side_col:
    st.markdown("<div style='height:36px;'></div>", unsafe_allow_html=True)
    panel2_equip = st.selectbox(
        "설비 선택",
        EQUIP_LIST["설비"].tolist(),
        index=EQUIP_LIST["설비"].tolist().index(selected_equip),
        key="panel2_equip",
    )

with chart_col:
    st.markdown('<div class="panel-card">', unsafe_allow_html=True)
    st.markdown('<div class="panel-title">2. Swap Delay Time Trend (sec/day) ℹ️</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="panel-sub">선택 설비 : {panel2_equip} | 교체일 : {tcr_date.strftime("%Y-%m-%d")}</div>', unsafe_allow_html=True)

    # 합성 트렌드(분 단위) → 초 단위로 변환
...
# ────────────────────────────────────────────────────────────────────────────
# 5. 트렌드 차트 2개 (Temp Alarm / Upper Chamber Temp)
# ────────────────────────────────────────────────────────────────────────────
c2, c3 = st.columns(2)
